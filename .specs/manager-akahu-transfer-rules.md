...
- Split the widened FDX model into explicit receipt/payment and transfer projections before building Task 4+ transfer behavior. `existingFdxTransactionIdEntries` now contains transfer-side entries, but receipt/payment pending helpers still consume the full collection; `decidePendingExactFingerprint` and `decideStalePendingEntries` can treat transfer FDX IDs with the Akahu pending prefix as ordinary pending receipt/payment entries, while `decidePendingToSettledMatch` had to add a one-off transfer skip. Make receipt/payment pending decisions accept only a narrowed receipt/payment FDX entry type or projection, and keep transfer entries behind a transfer-specific projection/index until transfer-specific pending logic owns them.
- Add focused regression tests proving receipt/payment pending exact, stale, and pending-to-settled decisions ignore inter-account transfer FDX entries, especially transfer FDX IDs that use the Akahu pending fingerprint prefix. This guards Task 3's "do not change sync behavior yet" boundary before transfer sync wiring starts.
- Simplify `buildManagerBankOrCashAccountSyncRead` by replacing the repeated receipt/payment/credit-transfer/debit-transfer null-or-empty checks with a small typed append helper or source descriptors. The current implementation is still readable, but it establishes a branchy copy-paste shape exactly where future transfer duplicate/merge logic will grow.
- Make `buildManagerBankOrCashAccountSyncRead` require `interAccountTransfers` explicitly instead of silently defaulting to `[]`; tests and callers that intentionally omit transfers should pass an empty array at the call site so a supposedly complete sync-read model cannot accidentally drop transfer data.
