...
### Task 3 Review: Sync Read Transfer Index Code Quality Follow-Up

Status: Completed.

- Completed: Split the widened FDX read model into explicit receipt/payment and transfer projections while preserving the existing common FDX entries/index for cross-object duplicate lookup. `decidePendingExactFingerprint`, `decideStalePendingEntries`, and `decidePendingToSettledMatch` now consume only the receipt/payment projection, and transfer entries remain available through `existingTransferFdxTransactionIdEntries`/`existingTransferFdxTransactionIdIndex` for transfer-specific sync logic.
- Completed: Added focused regression tests proving receipt/payment pending exact, stale, and pending-to-settled decisions ignore inter-account transfer FDX entries, including transfer IDs that use the Akahu pending fingerprint prefix. Validation: `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "packages/manager-api/tests/ManagerBatchPagination.test.ts"`, `pnpm --filter @app/manager-api build`, and `pnpm --filter website build`.
- Completed: Simplified `buildManagerBankOrCashAccountSyncRead` by replacing the repeated receipt/payment/credit-transfer/debit-transfer null-or-empty checks with a small typed append helper that preserves the existing receipt/payment projection and transfer credit/debit projection shapes.
- Completed: Made `buildManagerBankOrCashAccountSyncRead` require `interAccountTransfers` explicitly instead of silently defaulting to `[]`; tests and callers that intentionally omit transfers now pass an empty array at the call site so a supposedly complete sync-read model cannot accidentally drop transfer data.
- Consider a business-level transfer batch read/cache before multi-account sync grows further. `GET/api4/inter-account-transfer-batch` has no account filter, so the current per-account sync read can refetch every transfer page once per linked account and then filter locally each time. A single business-wide transfer scan projected per account would remove duplicated I/O and make the ownership boundary clearer.
- Strengthen website sync-flow mocks/assertions so adding the transfer batch endpoint cannot silently change high-level sync behavior: assert unsupported accounts make zero transfer-batch requests and supported empty syncs make the expected empty transfer read, preferably through a shared typed sync-read mock helper instead of raw endpoint stubs spreading through tests.
...
- Completed: Tightened settled mirror-merge handling so a transfer whose opposite-side FDX already equals the current Akahu FDX ID is not considered a mirrored candidate. The pure duplicate decision now treats that branch as an existing transfer duplicate, and website settled sync no longer has a duplicate-index merge write path that could write one Akahu FDX ID onto both transfer FDX fields.
- Completed: Added focused pure helper coverage and `ManagerSyncFlows` coverage proving the opposite-side current-FDX branch skips without `POST` or `PUT`, increments duplicate skips, and leaves merge counts unchanged.
- Validation: `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`, and `pnpm --filter website build` passed.
- Completed: Moved `buildManagerAkahuSettledMirroredTransferUpdatePayload` from website orchestration into `packages/manager-api/src/ManagerAkahuTransactionSync.ts`, beside the transfer payload, duplicate, and mirrored-candidate helpers. The helper exposes a strict Manager inter-account transfer PUT payload type requiring `key` and `value`, starts from the existing transfer read item, and updates only the current-side settled clear-status field plus the current-side FDX field.
- Completed: Replaced the website-local merge payload construction in `apps/website/src/Manager/SyncFlows.ts` with the manager-api helper, leaving website orchestration responsible only for choosing the write and executing the Manager client call.
- Completed: Added focused pure manager-api tests proving credit-side and debit-side settled merge payloads preserve unrelated transfer fields, opposite-side clear status/date, and opposite-side FDX fields.
- Future simplification opportunity: settled transfer write orchestration now composes duplicate detection and mirrored-candidate selection in nested website switches, producing create/duplicate/warning/merge paths in orchestration. Collapse this into one pure settled transfer write decision such as `create | merge | duplicateSkip | warningSkip`, carrying the Manager write payload where applicable. The website flow should increment match counts, execute the selected write, and update state, while manager-api owns the transfer-specific branching; this will also give Task 8 pending replacement a cleaner extension point instead of adding another special-case branch in `SyncFlows.ts`.
- Validation: `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`, and `pnpm --filter website build` passed.
