...
### Task 6 follow-up review: Make pending sync a first-class account phase

- Rename the hidden sync service/orchestrator/types away from settled-only names now that the flow imports both settled and pending transactions. `ManagerAkahuSettledSync...`, `syncManagerAkahuSettledTransactions`, and `ManagerAkahuSettledAccountProcessor...` now describe only half of the behavior, which will make Task 7 UI wiring and future pending-to-settled work harder to reason about. Prefer `ManagerAkahuTransactionSync...` / `syncManagerAkahuTransactions` naming, or split a neutral account orchestrator from explicit settled and pending phase processors. Because this service is still hidden/test-only, do not add compatibility aliases unless a concrete consumer requires them.
- Restructure `apps/website/src/Manager/SyncFlows.ts` so account orchestration owns the one-time Manager sync read and importability decision, then runs clearly separate settled and pending phases. Keep the five-overlap stop state local to the settled phase, keep shared counts/warnings/errors/processed fdx IDs in a neutral account state, and stop passing pending transactions through `ManagerAkahuSettledAccountProcessor...` types. The goal is to make the flow read as `sync account -> settled phase -> pending phase`, not as pending logic bolted onto a settled processor.
- Collapse the duplicated receipt/payment POST plumbing in `createManagerAkahuSettledTransaction` and `createManagerAkahuPendingTransaction` into one shared classified-create writer that selects the Manager endpoint, handles write errors, records the processed fdx ID, and applies caller-provided success counts. Add a similarly direct PUT dispatcher for exact pending updates. Endpoint selection and write-error handling should not be duplicated across settled receipt, settled payment, pending receipt, pending payment, and pending update branches.
- Introduce an explicit pending update policy/helper instead of reusing the create classification payload directly as the PUT value at the call site. The helper should make the update contract obvious: either intentionally replace the Manager entry with the canonical suspense payload, or merge/preserve user-editable fields that are safe to keep. Add tests for the chosen contract so exact pending updates do not silently expand into accidental full-entry replacement debt.
