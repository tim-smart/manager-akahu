...
### Task 7 controller extraction follow-up review: Make controller start atomic (pending)
...
- Normalize the sync runner boundary so synchronous throws are handled by the same failure path as rejected promises. Today `inFlightRef.current` is set before `runTransactionSync` is called, and the `.then` handlers are only installed if that call returns normally. If the atom runner or a future wrapper throws synchronously, the controller can leave the dialog stuck in `running` with the in-flight ref permanently true. Use `Promise.resolve().then(() => runTransactionSync({ accounts }))` or an equivalent helper, and keep the ref reset plus failed-state transition in one shared rejection/finalization path.
- Add focused controller tests for both invariants: an old/stale start callback cannot launch a sync after the current state is no longer confirming, and a synchronous runner throw moves the running dialog to failed while clearing the in-flight guard so a later valid start is possible.
- Validation: not rerun for this review-only specification update; review inspected `apps/website/src/Manager/useManagerAkahuSyncController.ts`, `apps/website/src/Manager/SyncUi.ts`, `apps/website/src/main.tsx`, `apps/website/tests/ManagerAkahuSyncController.test.ts`, `package.json`, `LALPH_REVIEW.md`, and the current Task 7 spec notes.
