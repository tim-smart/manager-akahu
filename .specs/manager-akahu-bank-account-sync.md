...
- The sync date boundary no longer accepts decoded `DateTime` values. Callers must pass either a tagged Manager `yyyy-mm-dd` date or a tagged preserved raw Akahu date string; raw Akahu dates preserve their leading calendar date for Manager payloads and pending fingerprints.
...
- Resolve the Akahu calendar-date preservation boundary before service wiring. `formatManagerAkahuDate` preserves near-midnight calendar dates only when given the original Akahu date string, but the existing domain/RPC models expose decoded `DateTime.Utc` values where the original offset/calendar spelling has already been lost. Either carry a raw Akahu calendar date/string through the domain/RPC boundary for sync, or narrow the helper API to an explicit `yyyy-mm-dd` Manager date so future callers cannot accidentally believe a `DateTime` preserves the Akahu calendar date. (completed with tagged Manager/raw Akahu string inputs)
