...
### Task 7 follow-up review: Extract sync UI controller and accessible dialog boundary (partially completed)

- Refactor `apps/website/src/main.tsx` so the application entrypoint stops owning the setup rendering, ready-account sync controls, sync lifecycle, and modal markup in one file. The Task 7 slice expanded `main.tsx` into the main UI/controller module, while an earlier setup-state review already called out that setup UI should move out before sync controls grew. Prefer focused Manager UI modules such as `Manager/SetupStateView.tsx`, `Manager/LinkedAccountsList.tsx`, `Manager/SyncDialog.tsx`, and a small controller hook/module so `App` only wires atoms to components. (partially completed for the sync lifecycle/controller hook; setup rendering, ready-account list, and modal markup still live in `main.tsx`)
