...
### Task 6 pending update policy follow-up: Collapse classified PUT update plumbing
...
- Prefer a direct helper/writer such as `putManagerAkahuClassifiedUpdate({ client, key, classification })` that owns receipt/payment endpoint selection, write-error mapping, and the canonical suspense replacement value. This deletes the duplicated `PUT/api4/receipt` / `PUT/api4/payment` branching in `updateManagerAkahuPendingTransaction` and `updateManagerAkahuSettledPendingReplacement` instead of adding a second thin update-shape abstraction.
- Tighten the update payload type boundary while refactoring. Avoid returning broad generated `PutReceipt` / `PutPayment` wrappers with optional `key` and `value` from the policy helper; use a local exact update payload type with required `key` and required canonical suspense receipt/payment value so the helper's contract matches the invariant it constructs.
