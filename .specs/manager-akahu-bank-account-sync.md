...
### Task 6 stale pending detection review: Move stale decision to the pure sync helper

- Refactor stale pending detection out of `apps/website/src/Manager/SyncFlows.ts` and into the canonical pure sync policy layer in `packages/manager-api/src/ManagerAkahuTransactionSync.ts`. The current implementation adds `currentPendingFdxTransactionIds` to the broad account state and scans stale entries locally in website orchestration, while settled duplicate, exact pending fingerprint, and pending-to-settled decisions already live in the pure helper module. Prefer a pure helper such as `decideStalePendingEntries({ syncRead, currentPendingFdxTransactionIds, processedFdxTransactionIds })` that returns the stale `ManagerExistingFdxTransactionIdEntry` values.
...
- Preserve first-pass behavior: do not delete or modify stale Manager entries, run stale detection only after a successful pending endpoint phase for accounts that support pending transactions, emit one clear non-fatal warning per stale entry, increment `stalePendingDetected`, and preserve existing summary behavior.
- Validation target: `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`, `pnpm --filter @app/manager-api build`, and `pnpm --filter website build`.
