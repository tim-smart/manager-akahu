...
### Task 7 accessible dialog boundary follow-up review: Localize trigger focus ownership
...
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM test harness is added at the same time, add focused coverage for trigger focus restoration and Radix focus trapping; otherwise preserve the existing test-harness limitation note.
