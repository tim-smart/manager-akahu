...
- Task 6 same-run pending-to-settled reuse follow-up filters pending replacement candidates against the current account state's processed `fdxTransactionId`s before calling `decidePendingToSettledMatch`. After a pending entry is replaced once in a settled stream, its old pending fingerprint is no longer eligible for later settled rows in the same account run, so later matching settled rows follow the normal zero-candidate create path.
...
### Task 6 same-run candidate reuse follow-up review: Move exclusion into the pure matcher

- Refactor the same-run exclusion out of `apps/website/src/Manager/SyncFlows.ts`. The current fix rebuilds a filtered `ManagerBankOrCashAccountSyncRead` in website orchestration before calling `decidePendingToSettledMatch`, which leaks pending-replacement matching policy out of the pure sync helper and adds an ad-hoc wrapper over the canonical read model. Future callers can still forget to apply the filtered view and reintroduce same-run pending candidate reuse.
- Prefer adding an explicit `excludedFdxTransactionIds` or `unavailableFdxTransactionIds` input to `decidePendingToSettledMatch` in `packages/manager-api/src/ManagerAkahuTransactionSync.ts`, then skip those entries inside the existing candidate loop. This makes the helper contract state the real invariant directly: processed pending fingerprints are not eligible replacement candidates.
- Delete `getManagerAkahuPendingToSettledCandidateSyncRead` and `shouldKeepManagerAkahuSyncReadItem` from `SyncFlows.ts` after the pure matcher owns the exclusion. Avoid rebuilding the canonical sync-read model per settled transaction solely to hide entries from one decision helper.
- Add focused pure-helper coverage for excluded pending candidates in `packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts`, and keep the existing website same-run test as orchestration coverage proving the first settled row updates and the second settled row creates normally without a second `PUT` to the same Manager key.
- Validation: run `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`, `pnpm --filter @app/manager-api build`, and `pnpm --filter website build`.
...
- Introduce an explicit pending update policy/helper instead of reusing the create classification payload directly as the PUT value at the call site. The helper should make the update contract obvious: either intentionally replace the Manager entry with the canonical suspense payload, or merge/preserve user-editable fields that are safe to keep. Add tests for the chosen contract so exact pending updates do not silently expand into accidental full-entry replacement debt. (not completed)
