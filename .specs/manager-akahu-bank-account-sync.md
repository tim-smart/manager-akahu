...
- Task 7 accessible dialog follow-up moved the sync modal markup into `apps/website/src/Manager/SyncDialog.tsx` and replaced the hand-rolled overlay with Radix `Dialog`. Radix owns modal focus trapping and background inertness, while local controlled `onOpenChange`, Escape, and pointer-outside guards preserve the running-state invariant. `App` records the clicked sync button and passes it to the dialog so close auto-focus restores focus to the triggering `Sync all` or per-account sync button.
...
### Task 7 trigger-boundary follow-up review: Make trigger ownership a real local control boundary (pending)

- Refactor the ready-state sync controls so setup/account rendering no longer imports or renders the dialog-specific `SyncDialogTriggerButton`. The current follow-up removes `HTMLButtonElement` callback plumbing, but `SetupStateView` and `Accounts` still know they are rendering Radix dialog triggers, so dialog mechanics remain leaked into setup/account components under a different abstraction name.
- Avoid making `SyncDialog` wrap the whole application/setup page solely so scattered triggers can live under the same `Dialog.Root`. Prefer a focused sync-control boundary, such as a ready linked-accounts sync section, that owns `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, and the dialog content together. `App` should wire atoms/controller state, while non-dialog setup/account components should stay domain-shaped or be moved inside that dedicated sync-control section.
- Delete or make private any exported trigger wrapper that only exists to smuggle Radix trigger semantics into unrelated render paths. The cleaner shape should make the sync dialog/control boundary the only place a reader needs to understand Radix trigger/focus restoration mechanics.
...
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM harness is added, also cover trigger focus restoration and focus trapping; otherwise keep the existing harness limitation explicit.
