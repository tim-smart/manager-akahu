...
### Task 6 stale pending helper review follow-up: Make pending fingerprint collection unavoidable (not completed)
...
- Delete `ManagerAkahuPendingPhaseStep.currentPendingFdxTransactionId` and the repeated branch returns that manually carry it through duplicate, ambiguous, create/update, zero, and unsupported-import paths. Stale-pending correctness should not depend on every future pending branch remembering to populate an optional field; unsupported fingerprint construction should be the only path that does not add a current pending fingerprint.
- Keep behavior unchanged: current pending fingerprints must still exclude entries from stale detection even when the pending row is a duplicate, ambiguous exact match, zero amount, unsupported Manager import, successfully created, or successfully updated; failed pending endpoint phases must still skip stale detection entirely.
- Add or adjust focused website orchestration coverage if needed to prove a fingerprinted-but-not-written current pending row, such as zero amount or unsupported Manager import, does not get reported as stale when an existing Manager entry with the same pending fingerprint is present.
- Validation target: `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"` and `pnpm --filter website build`.
