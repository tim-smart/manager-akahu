...
### Task 7 local control-boundary review: Collapse remaining sync action-slot plumbing (pending)

- Refactor the ready linked-account sync UI so `apps/website/src/main.tsx` no longer imports `SyncControlActions` or participates in placing dialog-owned button factory output. The current implementation makes `SyncDialogTriggerButton` private, but replaces the previous trigger leak with a render-prop/action-slot contract (`syncAllButton` / `accountSyncButton`) exported from `Manager/SyncDialog.tsx`. This keeps the sync-control boundary split across `SyncDialog.tsx` and `main.tsx`, so account rendering still knows about a dialog-owned action-node protocol rather than staying purely domain-shaped.
- Prefer a single focused ready-sync section component, such as `Manager/LinkedAccountsSyncSection.tsx` or `Manager/ReadySyncControls.tsx`, that owns the ready linked-account list layout, `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, and sync dialog content together. `App` should only branch on setup state and wire atom/controller state into that component; it should not thread dialog action factories through `ReadySetupState` and `Accounts`.
- Delete the exported `SyncControlActions` type and the render-prop button factories once the ready-sync section owns both trigger placement and dialog content. Keep generic setup-message/stale-selection helpers if useful, but avoid replacing the action-slot contract with another thin wrapper that still requires unrelated render paths to place dialog-owned React nodes.
- Preserve current behavior while collapsing the boundary: no `HTMLButtonElement` values in sync callbacks, Radix focus restoration from trigger close, running-state close guards for Escape/overlay/Close/Cancel, ready sync buttons disabled while running, duplicate-start prevention through the controller, and sanitized completed/failure summaries.
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM harness is added, also cover trigger click account selection, ready-button disabling while running, focus restoration/trapping, and rendered summary/warning/error output.
