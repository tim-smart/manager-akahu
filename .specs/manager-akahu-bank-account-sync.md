...
- apps/server/src/Akahu.ts uses a cursor-only settled transaction request boundary. Akahu's documented account settled-transaction endpoint defaults to the full app-accessible date range when `start`/`end` are omitted, so settled sync should not add a fixed last-30-days range.
...
- The transaction request shape was intentionally kept unchanged apart from forwarding `cursor`. Later documentation verification found this cursor-only Akahu account settled-transaction request is the desired settled-history boundary because omitting `start`/`end` requests the full app-accessible range rather than a fixed 30-day window.
...
- No settled older-history/date-window RPC boundary was added in this task. The settled transaction boundary remained the existing account transaction request with cursor pagination only; this was later verified as the correct older-history fetch mechanism because Akahu defaults that endpoint to the full app-accessible range when no `start`/`end` query is supplied.
...
- Task 5 foreign-currency skip follow-up lifted the existing Manager account currency importability decision to the selected-account orchestration boundary. Unsupported foreign-currency accounts now skip Manager receipt/payment sync reads and Manager writes, emit one account-level warning, and fetch only Akahu settled/pending streams needed for summary counts; fetched rows are counted as `unsupportedSkipped`, with pending still gated by Manager pending support and skipped after a settled endpoint failure.
- Task 5 foreign-currency skip follow-up review narrowed the normal settled/pending account phase context to an importable-only Manager bank/cash account currency decision. Unsupported foreign-currency skip decisions remain handled exclusively by the account-level unsupported summary path, while per-transaction unsupported handling remains for cases such as unsupported pending fingerprint construction.
...
- Stale pending detection runs after a successful pending endpoint phase for an account. It scans the initial complete Manager receipt/payment sync read for Akahu-created pending `fdxTransactionId` values, excludes fingerprints present in the current Akahu pending result set, and excludes entries already processed in the same run by pending create/update or pending-to-settled replacement.
...
- Task 6 stale pending follow-up detects prior Manager Akahu-created pending receipts/payments absent from the current Akahu pending endpoint result and not replaced by settled transactions in the same run. The first implementation does not delete or modify those Manager entries; it increments `stalePendingDetected`, adds one non-fatal warning per stale entry, and preserves existing warning summary behavior. Focused mocked tests cover stale pending receipt/payment reporting with no Manager writes and the settled-replacement exemption.
- Task 6 stale pending detection review moved stale selection into the pure manager-api helper `decideStalePendingEntries`, which consumes the canonical Manager sync read, the current pending endpoint fingerprint set, and processed `fdxTransactionId`s. Website orchestration now keeps the current pending fingerprint set local to the pending phase and translates the returned stale entries into the existing non-fatal warnings/counts after a successful pending endpoint phase.
- Task 6 pending fingerprint collection follow-up moved current pending fingerprint collection to the pending phase boundary. `syncManagerAkahuPendingTransactionPhase` now calls `buildAkahuPendingTransactionFingerprint` once per Akahu pending row, records successful fingerprints immediately in the current pending set, and passes the fingerprint decision into branch processing. The deleted per-branch `currentPendingFdxTransactionId` side channel means duplicate, ambiguous, zero-amount, unsupported-import, created, and updated pending rows all remain excluded from stale-pending detection, while failed pending endpoint phases still skip stale detection entirely.
...
- Task 7 controller extraction moved the modal lifecycle and current-tab in-flight guard into `apps/website/src/Manager/useManagerAkahuSyncController.ts`. `App` now wires the sync atom into this controller and renders controller state, while duplicate-start prevention is a single controller invariant that also handles the stale React state seen by rapid double-clicks.
- The focused controller tests live in `apps/website/tests/ManagerAkahuSyncController.test.ts`, with the root command `pnpm test:website-sync-controller`. They prove rapid double-start only invokes the sync mutation once and controller close paths cannot dismiss running state. The first attempted React hook test used `happy-dom`, but this workspace does not install `happy-dom`; the final test covers the exported controller actions without adding a DOM dependency.
- Task 7 controller start follow-up moved start selection into the controller-owned functional state transition. The start helper no longer accepts render-captured dialog state, so stale callbacks cannot launch after the current dialog state has left `confirming`. Synchronous throws from the sync runner now enter the same failed-state path as rejected promises, clear the in-flight guard, and allow a later valid start.
- Task 7 React-updater follow-up removed the updater side effect from `startManagerAkahuSyncController`. The controller now owns a synchronized `stateRef` plus local setter; start reads the ref, captures accounts, sets `running`, and updates the current-tab guard in one synchronous controller transaction before launching the sync runner. Deferred React-style setter tests prove start either queues `running` with exactly one launched sync or leaves state/guard unchanged with no launch.
- Task 7 accessible dialog follow-up moved the sync modal markup into `apps/website/src/Manager/SyncDialog.tsx` and replaced the hand-rolled overlay with Radix `Dialog`. Radix owns modal focus trapping and background inertness, while local controlled `onOpenChange`, Escape, and pointer-outside guards preserve the running-state invariant. Trigger focus restoration now stays within the Radix dialog/trigger boundary instead of passing clicked button elements through `App`.
- Completed sync summaries are sanitized before entering controller-owned UI state so credential-looking warning/error text such as Akahu token fields or Authorization headers is redacted before rendering. No actual Akahu token values are stored by the dialog/controller state.
- Focused DOM assertions for focus trapping and focus restoration could not be added because the current focused website test command runs Vitest with `environment 0ms` / Node and the workspace still does not install a DOM implementation such as `happy-dom` or `jsdom`. The new focused tests cover the pure running-state dismissal guard and UI-state redaction boundary; component-level focus tests remain a test-harness follow-up.
- Task 7 accessible dialog review found one remaining structural boundary issue: focus restoration is implemented by threading `HTMLButtonElement` through setup and account-list sync callbacks, so non-dialog components now know about the dialog's DOM focus mechanics. A follow-up should move trigger ownership into the sync-dialog/control boundary, preferably using Radix `Dialog.Trigger asChild` where feasible, so setup/account components return to a domain-shaped `onSync(accounts)` contract.
- Task 7 trigger-boundary follow-up moved the sync trigger buttons into `SyncDialogTriggerButton`, which composes the existing `Button` with Radix `Dialog.Trigger asChild` inside the same controlled `Dialog.Root` as the sync dialog content. Setup/account components now call domain-shaped `onSync(accounts)` callbacks and no longer pass `HTMLButtonElement` values through sync callbacks. Radix handles close focus restoration to the active trigger, while the existing controlled close guard, duplicate-start controller guard, and sanitized completion/failure state boundaries remain unchanged.
- Task 7 local control-boundary follow-up replaced the exported trigger wrapper with a private trigger inside `SyncControls`. `SyncControls` now owns `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, and the sync dialog content together, while `App` only wires atom/controller state and setup/account rendering consumes action slots instead of importing or rendering dialog-specific triggers. Validation passed with `pnpm test:website-sync-controller` and `pnpm --filter website build`; the website build still reports Vite's existing large-chunk warning.
- Task 7 action-slot collapse follow-up replaced the render-prop/action-slot ready sync contract with `apps/website/src/Manager/LinkedAccountsSyncSection.tsx`. The ready section now owns the linked-account list layout, controlled Radix `Dialog.Root`, `Dialog.Trigger asChild` sync buttons, selected-account opening, and dialog content placement. `SyncDialog.tsx` now exports only focused dialog content/guard markup, and shared setup display pieces live in `Manager/SetupUi.tsx`; `App` only branches on setup state and wires the sync controller into the ready section. Validation passed with `pnpm test:website-sync-controller` and `pnpm --filter website build`; the website build still reports Vite's existing large-chunk warning.
- Task 7 action-slot collapse review found no new actionable structural follow-up. Keep `LinkedAccountsSyncSection` as the ready-state boundary that owns linked-account rendering, selected-account trigger opening, controlled Radix `Dialog.Root`, and sync dialog content placement; do not reintroduce exported trigger wrappers, render-prop action slots, or dialog-owned React node factories through `main.tsx`. The remaining gap is still DOM-harness coverage for rendered trigger/focus/summary behavior, not another structural refactor.

### Settled-history boundary verification findings
...
### Task 5 foreign-currency skip follow-up: Lift skip to account boundary (completed)

- Move the existing Manager bank/cash account currency importability decision ahead of Manager receipt/payment sync reads in `apps/website/src/Manager/SyncFlows.ts`. Unsupported foreign-currency accounts now return through an account-level summary path instead of entering transaction classification, avoiding duplicate per-transaction unsupported warnings and unnecessary Manager reads/writes. (completed)
- Preserve summary usefulness by fetching Akahu settled transactions and, when `canHavePendingTransactions` is true and the settled phase did not fail, Akahu pending transactions solely to count fetched rows as `unsupportedSkipped`. Transactions not fetched are not counted. (completed)
...
### Task 5 foreign-currency skip follow-up review: Narrow the importable account phase contract (completed)

- Tighten `apps/website/src/Manager/SyncFlows.ts` after the account-level currency gate so normal settled/pending phases can only be entered for importable Manager accounts. The previous `ManagerAkahuTransactionSyncAccountContext.importabilityDecision: ReturnType<typeof getManagerBankAccountCurrencyImportDecision>` still permitted a skip decision even though `syncManagerAkahuTransactionsForAccount` returned early for skipped foreign-currency accounts, leaving the old per-transaction unsupported-account path structurally reachable. (completed with a source-local `Extract<ManagerBankAccountCurrencyImportDecision, { _tag: "import" }>` context contract)
- Prefer an importable-only context contract, such as `Extract<ManagerBankAccountCurrencyImportDecision, { _tag: "import" }>` or a small source-local importable decision value, and pass that into `classifyManagerAkahuSuspenseImport` from the normal phases. Keep unsupported foreign-currency counting/warnings exclusively in the account-level unsupported summary path. (completed)
- Preserve real per-transaction unsupported handling that is still meaningful, such as invalid amount or unsupported pending fingerprint construction, but avoid a contract where a future caller can accidentally reintroduce duplicate per-transaction foreign-currency warnings by passing a skip decision into the supported-account processors. (completed; focused mocked coverage now asserts unsupported pending fingerprint construction still emits a per-transaction warning for an importable account)
- Add focused type or mocked coverage if useful to prove the normal phase context cannot carry a skip currency decision while the unsupported account path still emits one account-level warning and no Manager reads/writes. (completed with the context type boundary plus existing unsupported foreign-currency mocked coverage and added importable-account unsupported pending fingerprint coverage)
- Validation: `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"` and `pnpm --filter website build` pass for this follow-up. Website build reports Vite's existing large-chunk warning.

### Task 5 foreign-currency skip follow-up review follow-up: Delete residual import token and clean coverage boundary (pending)

- Refactor `apps/website/src/Manager/SyncFlows.ts` so `ManagerAkahuTransactionSyncAccountContext` no longer carries `importabilityDecision` at all. After the account-level foreign-currency gate, normal settled/pending phases have only one possible Manager currency policy: import. The current source-local `Extract<ManagerBankAccountCurrencyImportDecision, { _tag: "import" }>` narrows the type but still threads a payload-free policy token through account context, settled processing, and pending processing as if it were real per-account runtime state. Prefer a module-local import decision constant at the two `classifyManagerAkahuSuspenseImport` call sites, or a tiny importable-account classification wrapper, so the normal context contains only true per-account dependencies: account, client, and sync read.
- Delete the now-unnecessary `ManagerAkahuImportableCurrencyDecision` alias and the `const importableDecision: ManagerAkahuImportableCurrencyDecision = importabilityDecision` handoff in `syncManagerAkahuTransactionsForAccount`. The account-level `_tag === "skip"` branch should be the only place that observes a skip currency decision; normal phases should not preserve an abstraction whose only runtime value is `{ _tag: "import" }`.
- Replace the cast-backed website test fixture `makeUnsupportedAmountPendingTransaction(... ) as unknown as PendingTransaction` with boundary-appropriate coverage. Production website sync receives domain-decoded `PendingTransaction` values whose `amount` is a `BigDecimal`, so an invalid string amount is not a realistic website orchestration input. Move malformed pending amount coverage to the pure manager-api helper boundary, where `ManagerAkahuDecimalInput` intentionally accepts strings, or make the website sync input boundary explicitly raw/defensive if it truly must accept undecoded pending rows.
- Keep website mocked coverage focused on real orchestration invariants: unsupported foreign-currency accounts emit one account-level warning, avoid Manager receipt/payment reads and writes, and normal importable account phases process domain-valid settled/pending transactions without admitting a skip currency decision.
- Validation target: `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`; if malformed amount coverage moves to `packages/manager-api`, also run `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`; finish with `pnpm --filter website build`.
...
- Add mocked tests for unsupported pending accounts, pending create/update, repeat pending sync without duplicates, stale pending detection, and safe pending-to-settled replacement. (completed for unsupported pending accounts, pending create/update, repeat pending sync without duplicates, stale pending detection, safe pending-to-settled replacement, and ambiguous/non-matching pending-to-settled fallbacks)
- Validation: `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"` and `pnpm --filter website build` pass.

### Task 6 stale pending detection review: Move stale decision to the pure sync helper (completed)

- Refactor stale pending detection out of `apps/website/src/Manager/SyncFlows.ts` and into the canonical pure sync policy layer in `packages/manager-api/src/ManagerAkahuTransactionSync.ts`. The current implementation adds `currentPendingFdxTransactionIds` to the broad account state and scans stale entries locally in website orchestration, while settled duplicate, exact pending fingerprint, and pending-to-settled decisions already live in the pure helper module. Prefer a pure helper such as `decideStalePendingEntries({ syncRead, currentPendingFdxTransactionIds, processedFdxTransactionIds })` that returns the stale `ManagerExistingFdxTransactionIdEntry` values. (completed)
- Keep the observed current-pending fingerprint set local to the pending phase instead of storing it on `ManagerAkahuTransactionSyncAccountState`. That set is phase-local input for stale detection, not durable account output state like counts, warnings, errors, or processed `fdxTransactionId`s. The website phase should collect fingerprints while processing the Akahu pending stream, call the pure stale detector after a successful pending endpoint phase, and only then translate returned stale entries into warnings and `stalePendingDetected` counts. (completed)
- Add focused pure-helper coverage in `packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts` for stale pending selection: ignore non-Akahu-pending IDs, ignore fingerprints present in the current pending endpoint result, ignore processed/replaced pending IDs, and return prior Akahu-created pending receipt/payment entries that are absent from the current pending set. Keep the existing mocked website tests as orchestration coverage for no Manager writes and the settled-replacement exemption. (completed)
- Preserve first-pass behavior: do not delete or modify stale Manager entries, run stale detection only after a successful pending endpoint phase for accounts that support pending transactions, emit one clear non-fatal warning per stale entry, increment `stalePendingDetected`, and preserve existing summary behavior. (completed)
- Validation: `pnpm --filter @app/domain build` was needed first to refresh local `packages/domain/dist`; after that, `pnpm test "packages/manager-api/tests/ManagerAkahuTransactionSync.test.ts"`, `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"`, `pnpm --filter @app/manager-api build`, and `pnpm --filter website build` pass. Website build reports Vite's existing large-chunk warning.

### Task 6 stale pending helper review follow-up: Make pending fingerprint collection unavoidable (completed)

- Refactor `apps/website/src/Manager/SyncFlows.ts` so the current Akahu pending fingerprint set is collected exactly once at the pending phase boundary, immediately after `buildAkahuPendingTransactionFingerprint` succeeds, instead of being returned as an optional `currentPendingFdxTransactionId` side channel from `processManagerAkahuPendingTransaction`. (completed)
- Delete `ManagerAkahuPendingPhaseStep.currentPendingFdxTransactionId` and the repeated branch returns that manually carry it through duplicate, ambiguous, create/update, zero, and unsupported-import paths. Stale-pending correctness should not depend on every future pending branch remembering to populate an optional field; unsupported fingerprint construction should be the only path that does not add a current pending fingerprint. (completed)
- Keep behavior unchanged: current pending fingerprints must still exclude entries from stale detection even when the pending row is a duplicate, ambiguous exact match, zero amount, unsupported Manager import, successfully created, or successfully updated; failed pending endpoint phases must still skip stale detection entirely. (completed)
- Add or adjust focused website orchestration coverage if needed to prove a fingerprinted-but-not-written current pending row, such as zero amount or unsupported Manager import, does not get reported as stale when an existing Manager entry with the same pending fingerprint is present. (completed with a zero-amount pending row)
...
### Task 6 pending fingerprint collection audit: Accept phase-boundary collection (completed)

- Deep code-quality review found no new actionable structural follow-up for the pending fingerprint collection change. Keep current pending fingerprint collection at the pending phase boundary in `syncManagerAkahuPendingTransactionPhase`, immediately after `buildAkahuPendingTransactionFingerprint` returns a fingerprint, and keep `processManagerAkahuPendingTransaction` returning only the updated account state rather than reintroducing an optional per-branch `currentPendingFdxTransactionId` side channel. (completed)
- Preserve the current zero-amount orchestration coverage proving a fingerprinted-but-not-written pending row is excluded from stale-pending detection. The broader pending import-candidate simplification remains the already-tracked unresolved Task 6 design item; this audit does not add a duplicate follow-up for it. (completed)
- Validation: not rerun for this review-only specification update; the reviewed task already records `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"` and `pnpm --filter website build` passing.
...
- Show all summary counts and warnings/errors without exposing credentials. (completed; the UI state does not carry Akahu token values and controller completion sanitizes credential-looking warning/error text before rendering)
...
### Task 7 follow-up review: Extract sync UI controller and accessible dialog boundary (completed for sync controller/dialog)

- Refactor `apps/website/src/main.tsx` so the application entrypoint stops owning the setup rendering, ready-account sync controls, sync lifecycle, and modal markup in one file. The Task 7 slice expanded `main.tsx` into the main UI/controller module, while an earlier setup-state review already called out that setup UI should move out before sync controls grew. Prefer focused Manager UI modules such as `Manager/SetupStateView.tsx`, `Manager/LinkedAccountsList.tsx`, `Manager/SyncDialog.tsx`, and a small controller hook/module so `App` only wires atoms to components. (completed for sync lifecycle/controller hook and modal markup extraction to `Manager/SyncDialog.tsx`; setup rendering and ready-account list still live in `main.tsx` as a separate setup-UI extraction follow-up)
- Make current-tab sync concurrency a single canonical controller invariant rather than a split between `SyncUi.ts` pure transitions and a component-local `syncInFlightRef` in `App`. `akahuTransactionSyncAtom` is still a bare callable mutation, so future sync triggers can bypass the ref guard. Prefer a `useManagerAkahuSyncController` or equivalent boundary that owns `open`, atomic `confirming -> running` start, in-flight guarding, completion/failure transitions, and guarded close behavior; components should render controller state instead of duplicating lifecycle checks. (completed with `useManagerAkahuSyncController`; pure `SyncUi.ts` transitions remain as the dialog-state model, but the in-flight concurrency guard is no longer owned by `main.tsx`)
- Replace the hand-rolled `role="dialog"` overlay in `main.tsx` with a real accessible dialog primitive, preferably a shadcn/Radix `Dialog` or a tiny local primitive built on the same focus-management layer. The current modal focuses one button but does not trap focus, inert background content, or restore focus to the triggering sync button after close. Preserve the running-state close guard through `onOpenChange`/dismiss handling so Escape, overlay click, close, and cancel remain disabled while a sync is running. (completed with Radix `Dialog` in `Manager/SyncDialog.tsx`; controlled close, Escape, and pointer-outside handlers all preserve the running-state guard, and close auto-focus restores through the Radix trigger boundary)
- Move Task 7 UI tests out of the service-flow test grab bag and cover the behavior at the layer where it exists. The current tests only exercise pure `SyncUi.ts` reducers, not React wiring. Add focused component/controller tests proving double-click Start invokes the sync mutation once, ready-state sync buttons disable while running, Escape/overlay/Close cannot dismiss during running, completed summaries render warnings/errors/counts, and focus behavior is managed by the dialog primitive. (partially completed with `apps/website/tests/ManagerAkahuSyncController.test.ts` covering double-start, controller close guards, the pure primitive-dismissal close transition, and UI-state redaction; ready-button disabling, rendered-summary component assertions, and focus behavior remain open until a DOM test harness such as `happy-dom` or `jsdom` is available)
- Preserve the first-slice behavior while refactoring: invoke `ManagerSyncFlows.syncTransactions`, keep credentials out of UI state/errors, keep no cross-tab locking, keep no live per-transaction progress until the service streams progress, and keep `pnpm test "apps/website/tests/ManagerSyncFlows.test.ts"` plus `pnpm --filter website build` passing. Add any new focused UI test command to the validation notes. (completed for this controller/dialog extraction with `pnpm test:website-sync-controller` and `pnpm --filter website build`; website build still reports Vite's existing large-chunk warning)

### Task 7 accessible dialog boundary follow-up review: Localize trigger focus ownership (completed)

- Refactor the accessible sync dialog wiring so focus restoration does not require `main.tsx`, `SetupStateView`, or `Accounts` to traffic in `HTMLButtonElement`. The current implementation passes the clicked button through the sync callback solely so `App` can stash it in `syncTriggerRef` and pass it back into `SyncDialog`, which leaks dialog/focus mechanics through otherwise domain-shaped setup/account rendering. (completed)
- Prefer a small sync-dialog/control boundary that owns both the sync trigger buttons and the Radix dialog content. If practical, compose each `Sync all` / per-account sync button with `Dialog.Trigger asChild` so Radix can restore focus to the trigger without a manual `restoreFocusElement` prop. If controlled selected-account state makes direct triggers awkward, keep the trigger ref local to that boundary and expose only `onSync(accounts)` or an equivalent account-selection action to setup components. (completed with `SyncDialogTriggerButton` using `Dialog.Trigger asChild`)
- Keep the existing running-state guard and behavior intact: Escape, overlay click, Close, and Cancel must still be unable to dismiss while the sync is running; duplicate starts must remain blocked by the controller; completed summaries and failure text must remain sanitized before entering rendered UI state. (completed)
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM test harness is added at the same time, add focused coverage for trigger focus restoration and Radix focus trapping; otherwise preserve the existing test-harness limitation note. (completed; no DOM test harness was added, so the existing limitation remains)

### Task 7 trigger-boundary follow-up review: Make trigger ownership a real local control boundary (completed)

- Refactor the ready-state sync controls so setup/account rendering no longer imports or renders the dialog-specific `SyncDialogTriggerButton`. The current follow-up removes `HTMLButtonElement` callback plumbing, but `SetupStateView` and `Accounts` still know they are rendering Radix dialog triggers, so dialog mechanics remain leaked into setup/account components under a different abstraction name. (completed with `SyncControls` action slots; `SyncDialogTriggerButton` is private to the sync-control boundary)
- Avoid making `SyncDialog` wrap the whole application/setup page solely so scattered triggers can live under the same `Dialog.Root`. Prefer a focused sync-control boundary, such as a ready linked-accounts sync section, that owns `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, and the dialog content together. `App` should wire atoms/controller state, while non-dialog setup/account components should stay domain-shaped or be moved inside that dedicated sync-control section. (completed with `SyncControls` owning the controlled Radix root/content and trigger/open behavior while `App` wires controller state)
- Delete or make private any exported trigger wrapper that only exists to smuggle Radix trigger semantics into unrelated render paths. The cleaner shape should make the sync dialog/control boundary the only place a reader needs to understand Radix trigger/focus restoration mechanics. (completed; the trigger wrapper is private)
- Preserve existing behavior while restructuring: no `HTMLButtonElement` values in sync callbacks, Radix focus restoration from trigger close, running-state close guards for Escape/overlay/Close/Cancel, ready sync buttons disabled while running, duplicate-start prevention through the controller, and sanitized completed/failure summaries. (completed)
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM harness is added, also cover trigger focus restoration and focus trapping; otherwise keep the existing harness limitation explicit. (completed; no DOM harness was added, and website build still reports Vite's existing large-chunk warning)

### Task 7 local control-boundary review: Collapse remaining sync action-slot plumbing (completed)

- Refactor the ready linked-account sync UI so `apps/website/src/main.tsx` no longer imports `SyncControlActions` or participates in placing dialog-owned button factory output. The current implementation makes `SyncDialogTriggerButton` private, but replaces the previous trigger leak with a render-prop/action-slot contract (`syncAllButton` / `accountSyncButton`) exported from `Manager/SyncDialog.tsx`. This keeps the sync-control boundary split across `SyncDialog.tsx` and `main.tsx`, so account rendering still knows about a dialog-owned action-node protocol rather than staying purely domain-shaped. (completed)
- Prefer a single focused ready-sync section component, such as `Manager/LinkedAccountsSyncSection.tsx` or `Manager/ReadySyncControls.tsx`, that owns the ready linked-account list layout, `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, and sync dialog content together. `App` should only branch on setup state and wire atom/controller state into that component; it should not thread dialog action factories through `ReadySetupState` and `Accounts`. (completed with `Manager/LinkedAccountsSyncSection.tsx`)
- Delete the exported `SyncControlActions` type and the render-prop button factories once the ready-sync section owns both trigger placement and dialog content. Keep generic setup-message/stale-selection helpers if useful, but avoid replacing the action-slot contract with another thin wrapper that still requires unrelated render paths to place dialog-owned React nodes. (completed; `SyncDialog.tsx` now exports focused dialog content only, with shared setup display helpers moved to `Manager/SetupUi.tsx`)
- Preserve current behavior while collapsing the boundary: no `HTMLButtonElement` values in sync callbacks, Radix focus restoration from trigger close, running-state close guards for Escape/overlay/Close/Cancel, ready sync buttons disabled while running, duplicate-start prevention through the controller, and sanitized completed/failure summaries. (completed)
- Validation target after the refactor: `pnpm test:website-sync-controller` and `pnpm --filter website build`. If a DOM harness is added, also cover trigger click account selection, ready-button disabling while running, focus restoration/trapping, and rendered summary/warning/error output. (completed; no DOM harness was added, and website build still reports Vite's existing large-chunk warning)

### Task 7 action-slot collapse review: Accept ready sync section boundary (completed)

- Deep code-quality review found no new actionable structural follow-up for the ready linked-account sync section. The current shape deletes the prior render-prop/action-slot plumbing instead of moving it elsewhere: `LinkedAccountsSyncSection` owns the ready linked-account list, controlled Radix `Dialog.Root`, `Dialog.Trigger asChild` buttons, selected-account opening, disabled-running trigger state, and `SyncDialogContent` placement, while `App` only branches on setup state and wires controller callbacks.
- Keep `SyncDialog.tsx` focused on dialog content, guarded dismissal, focus target selection, and summary rendering. Do not reintroduce `SyncControlActions`, exported trigger wrappers, `syncAllButton` / `accountSyncButton` factories, or `HTMLButtonElement` callback plumbing unless a new concrete consumer requires a different boundary.
- Residual validation gap remains the existing test-harness limitation: without `happy-dom` or `jsdom`, focused tests still do not assert trigger account selection through rendered buttons, ready-button disabling while running, Radix focus restoration/trapping, or rendered summary/warning/error output. Treat that as a future DOM test harness task rather than a reason to restructure this boundary again.
...
### Task 7 controller extraction follow-up review: Make controller start atomic (completed)

- Refactor `startManagerAkahuSyncController` so the start decision, account capture, in-flight guard, and `confirming -> running` transition happen from the current dialog state in one functional state transition instead of from a render-captured `state` snapshot. The extracted controller removed the guard from `main.tsx`, but `start` still passes the hook's current render state into the helper; that leaves the invariant split across stale closure state, `inFlightRef`, and `SyncUi.ts` transition helpers. Prefer a small controller-owned transition that only captures accounts when the actual current state is `confirming` and the ref is clear, then launches the sync after the state transition has selected those accounts. (completed)
- Normalize the sync runner boundary so synchronous throws are handled by the same failure path as rejected promises. Today `inFlightRef.current` is set before `runTransactionSync` is called, and the `.then` handlers are only installed if that call returns normally. If the atom runner or a future wrapper throws synchronously, the controller can leave the dialog stuck in `running` with the in-flight ref permanently true. Use `Promise.resolve().then(() => runTransactionSync({ accounts }))` or an equivalent helper, and keep the ref reset plus failed-state transition in one shared rejection/finalization path. (completed)
...
### Task 7 controller start atomic follow-up review: Remove React updater side effects (completed)

- Rework `startManagerAkahuSyncController` so it no longer communicates selected accounts out of a React functional state updater through a mutable local variable. The current implementation captures `accounts` inside `setState((current) => { ... })` and expects that updater to have run synchronously before deciding whether to launch the runner. React updater functions must be pure and may be deferred, replayed, or double-invoked; this makes the controller scheduling-dependent and can enqueue a `running` transition without launching the sync or setting the guard. (completed with a controller-owned `stateRef` and value-only local setter)
- Prefer a controller-owned synchronous state boundary instead of preserving both an impure updater and `inFlightRef`. A cleaner shape is to keep a `stateRef` synchronized with React state through one local setter, derive the start decision from `stateRef.current`, update the ref/state to `running` synchronously, and launch the runner with the captured accounts. If a separate in-flight ref remains, the guard must be set in the same imperative controller transaction as the current-state decision, not after a React updater side effect. An equally acceptable reducer/command model may return `{ state, command }` from a pure transition and execute the command outside React. (completed; `inFlightRef` remains but is updated in the same synchronous start transaction as the `stateRef` transition)
- Keep the synchronous throw recovery added by the completed task, but collapse the duplicated moving pieces where possible. With a controller-owned current-state ref, `running` can become the canonical current-tab guard and the extra boolean may be unnecessary; avoid replacing the updater side effect with another thin wrapper that still leaves state and guard ownership split. (completed; synchronous throws and rejected promises still transition through the shared failed path and clear the guard)
...
### Task 8: Verify settled full-history Akahu boundary (completed)

- Verify Akahu settled account transaction reads are not limited to a fixed last-30-days window. Akahu documents `GET /accounts/{id}/transactions` as returning the full app-accessible date range by default when `start` and `end` are omitted, so the correct sync boundary is cursor-only settled reads plus the existing five-overlap stop logic. (completed)
- Make the server boundary explicit without adding a user-configurable date range. `apps/server/src/Akahu.ts` now routes settled reads through the named cursor-only `settledTransactionHistoryQuery`, while pending reads remain on the pending endpoint with `amount_as_number=true`. (completed)
