...
- The pinned Effect runtime's `Stream.runForEachWhile` stream adapter stops after a completed chunk in the observed local validation path. Settled sync now rechunks to singleton items and applies the processor's explicit stop result at that boundary so multi-pull failures and five-overlap stops remain observable under the current runtime.
